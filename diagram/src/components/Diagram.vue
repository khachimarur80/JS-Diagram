...
export default {
  ...
  methods : {
    ...
    generateTile(x, y, tileSize) {
      ...
      this.canvas.ctx.strokeStyle = "#222";
    ...
    /*positionNodes() {
      ...
      const nodes = [...this.treeNode.nodes];
      const edges = [...this.treeNode.edges];

      // Initialize node positions randomly
      nodes.forEach(node => {
        const newNode = new Node();
          newNode.text = node.id;
          newNode.x = Math.floor(Math.random() * 18) - 9;
          newNode.y = Math.floor(Math.random() * 12) - 6;
          this.nodes.push(newNode);
        });

      // Fruchterman-Reingold force-directed layout simulation
      const k = 1.5; // Optimal distance factor
      const c = 0.2; // Force factor
      ...
      for (let iter = 0; iter < iterations; iter++) {
        ...
        edges.forEach(edge => {
          const node1 = this.nodes.find(node => node.text === edge[0]);
          const node2 = this.nodes.find(node => node.text === edge[1]);
          ...
          const index1 = this.nodes.findIndex(node => node.text === edge[0]);
          const index2 = this.nodes.findIndex(node => node.text === edge[1]);
      ...
      // Center the nodes at (0, 0)
      this.nodes.forEach(node => {
        node.x -= centroid.x;
        node.y -= centroid.y;
      });

      // Create edges using calculated node positions
      edges.forEach(edge => {
        const node1 = this.nodes.find(node => node.text === edge[0]);
        const node2 = this.nodes.find(node => node.text === edge[1]);
    ...
    positionNodes() {
      ...
      this.treeNode.edges.forEach((edge)=>{
        let node1 = this.nodes.filter(node => node.text == edge[0])[0]
        let node2 = this.nodes.filter(node => node.text == edge[1])[0]
      ...
      function drawSubgraph(node, adjacentNodes, nodes, vm) {
        ...
        for (let i in adjacentNodes) {
          console.log(adjacentNodes[i].text)
          console.log(vm.incest(adjacentNodes[i]), adjacentNodes)
